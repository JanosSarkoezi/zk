...
\begin{enumerate}
    ...
    $$F(x) = \int_a^x f(t) \, dt$$
    definierte Integralfunktion eine [[stammfunktion_definition|Stammfunktion]] von $f$ auf $[a, b]$, d.h. $F'(x) = f(x)$.

    \item **Teil (Berechnung von Integralen)**: Ist $F$ eine beliebige [[stammfunktion_definition|Stammfunktion]] der R-integrierbaren Funktion $f$ auf $[a, b]$, so gilt:
