...
wobei $A(x) = \int a(x) \, dx$ eine [[stammfunktion_definition|Stammfunktion]] von $a$ ist.

--proof--
Die homogene Gleichung $y' = a(x)y$ ist eine [[dgl_getrennte_veraenderliche|DGL mit getrennten Veränderlichen]] und hat die Lösung $y_h(x) = C e^{A(x)}$. Für die partikuläre Lösung nutzt man den Ansatz der **Variation der Konstanten**: $y_p(x) = C(x) e^{A(x)}$. Einsetzen in die inhomogene Gleichung liefert $C'(x) e^{A(x)} = s(x)$, woraus $C(x) = \int s(x) e^{-A(x)} \, dx$ folgt.
